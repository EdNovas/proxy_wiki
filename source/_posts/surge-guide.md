...
| 平台 | 价格 | 说明 |
|------|------|------|
| iOS（Surge for iOS） | $49.99 | App Store 免费下载，应用内或官网购买 Pro 授权；一次性付费、永久使用，含一年功能更新订阅，之后解锁新功能需续订（$14.99/年），可用于 3 台设备（绑定 iCloud 后可免费扩展到 6 台） |
| macOS（Surge for Mac） | $49.99 起 | 官网购买授权，按设备数量定价（1 台 $49.99 / 3 台 $69.99 / 5 台 $99.99），含一年免费更新，之后需续费才能获得新版本 |
| 组合购买 | 各自独立 | iOS 和 macOS 版本需要分别购买 |

也就是说，如果你想在 iPhone 和 Mac 上同时使用 Surge，首次购买就要花费约 **100 美元**（$49.99 + $49.99），想持续获得新功能还要每年续费。这个价格放在代理工具领域是相当昂贵的——对比来看，Clash Verge 完全免费，Quantumult X 也只要 $9.99。
...
4. **持续更新**：购买后一年内免费获得更新，之后可按年续费；不续费也能永久使用已解锁的功能（Mac 版为到期前的最后一个版本），Surge 5 的更新频率保持在每月多次
...
- **Shadowsocks**
- **VMess**（官方手册与商店简介均未列出 VLESS）
- **Trojan**
- **Hysteria 2 / TUIC / AnyTLS**
- **HTTPS/SOCKS5 代理**
- **WireGuard**（内置支持）
- **Snell**（Surge 自研协议）
...
iOS 版 Pro 授权是个人授权，最多可同时在 3 台自己的 iOS 设备上使用，绑定 iCloud 账户后可免费扩展到 6 台。macOS 版的授权按设备数量计费（1 / 3 / 5 台）。

### Q4：Surge 和 Quantumult X、Loon 怎么选？

这三者都是 iOS 平台的高级代理工具。Quantumult X（$9.99）和 Loon（$7.99）价格更低，功能也不弱。对于大多数 iOS 用户来说，Quantumult X 的性价比最高。Surge 适合对网络调试和脚本有深度需求的用户。
