...
不同的代理客户端支持的规则类型略有差异，但以下几类是几乎所有主流客户端都支持的。
...
客户端内置了 IP 地理位置数据库（通常是 MaxMind 的 GeoIP 数据库或 GeoLite2），通过查询数据库来判断 IP 的所属国家。
...
- **Loyalsoldier**：基于 v2ray/Clash 的规则集，分类清晰，更新及时。
