...
Clash Meta（mihomo 内核）是目前桌面端使用最广泛的代理客户端之一，Clash Verge Rev 则是其最流行的图形界面。下面是推荐的 DNS 配置。
...
**正确的做法**：在 Fake-IP 模式下，删掉所有 `fallback` 和 `fallback-filter` 相关配置，只保留 `nameserver`，填入可靠的国内 DNS（推荐腾讯 DoH 或阿里 DoH）。
